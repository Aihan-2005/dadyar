...
function jalaliToGregorian(j_y: number, j_m: number, j_d: number): [number, number, number] {
  ...
  export function AddReminderForm({ onClose }: AddReminderFormProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      // handle both target by creating two separate reminders (lawyer + client)
      if (target === "both") {
        scheduledDates.forEach((scheduledFor) => {
          const payload: CreateReminderPayload = {
            title,
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              type="button"
              onClick={() => setTarget("lawyer")}
              className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border-2 transition-all ${target === "lawyer"
                  ? "border-blue-600 bg-blue-50 text-blue-700"
                  : "border-gray-200 text-gray-600 hover:border-gray-300"
                }`}
            >
            ...
            <button
              type="button"
              onClick={() => setTarget("client")}
              className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border-2 transition-all ${target === "client"
                  ? "border-green-600 bg-green-50 text-green-700"
                  : "border-gray-200 text-gray-600 hover:border-gray-300"
                }`}
            >
            ...
            <button
              type="button"
              onClick={() => setTarget("both")}
              className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border-2 transition-all ${target === "both"
                  ? "border-purple-600 bg-purple-50 text-purple-700"
                  : "border-gray-200 text-gray-600 hover:border-gray-300"
                }`}
            >
          ...
          <div className="flex gap-2">
            {[
              ...
              <button
                ...
                type="button"
                onClick={() => setPriority(item.value as NotificationPriority)}
                className={`flex-1 px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${priority === item.value
                    ? `${item.color}`
                    : "border-gray-200 text-gray-600 hover:border-gray-300"
                  }`}
              >
