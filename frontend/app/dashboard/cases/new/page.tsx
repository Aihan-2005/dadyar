...
export default function NewCasePage() {
  ...
  const [courtBranch, setCourtBranch] = useState<CourtBranch | undefined>(undefined)
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg border text-zinc-900 border-zinc-200 p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-900 mb-2">نام موکل *</label>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* شماره بایگانی دفتر — ثابت */}
            <div>
              <label className="block text-sm font-medium text-zinc-900 mb-2">
                شماره بایگانی دفتر
                <span className="mr-2 text-xs text-zinc-400 font-normal">(از پروفایل وکیل)</span>
              </label>
              <input
                type="text"
                value={(user as any)?.archiveNumberOffice || ''}
                disabled
                className="w-full px-4 py-2 border border-zinc-200 rounded-lg bg-zinc-50 text-zinc-500 cursor-not-allowed"
                placeholder="در پروفایل تنظیم کنید"
              />
            </div>

            {/* شماره بایگانی وکیل — ثابت */}
            <div>
              <label className="block text-sm font-medium text-zinc-900 mb-2">
                شماره بایگانی وکیل
                <span className="mr-2 text-xs text-zinc-400 font-normal">(از پروفایل وکیل)</span>
              ...
              <input
                type="text"
                value={(user as any)?.archiveNumberLawyer || ''}
                disabled
                className="w-full px-4 py-2 border border-zinc-200 rounded-lg bg-zinc-50 text-zinc-500 cursor-not-allowed"
                placeholder="در پروفایل تنظیم کنید"
              />
          ...
          <div>
            ...
            <input {...register('caseNumber')} type="text"
              className="w-full px-4 py-2 border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-900"
              placeholder="مثال: ۱۴۰۳/۵۵۲۱" />
            {errors.caseNumber && <p className="mt-1 text-sm text-red-600">{errors.caseNumber.message}</p>}
          </div>

          {/* شماره بایگانی شعبه + انتخاب شعبه */}
          <div>
            <label className="block text-sm font-medium text-zinc-900 mb-2">شعبه دادگاه</label>
          ...
          <div>
            ...
            <input {...register('archiveNumberBranch')} type="text"
              className="w-full px-4 py-2 border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-900"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-900 mb-2">
                وکیل هم‌رزم
                <span className="mr-2 text-xs text-zinc-400 font-normal">(همکار در پرونده)</span>
              </label>
              <input {...register('coLawyerName')} type="text"
                className="w-full px-4 py-2 border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-900"
                placeholder="نام وکیل هم‌رزم (اختیاری)" />
            </div>

            <div>
              <label className="block text-sm font-medium text-zinc-900 mb-2">
                وکیل هم توی رزمه
                <span className="mr-2 text-xs text-zinc-400 font-normal">(وکیل طرف مقابل یا دیگری در پرونده)</span>
              </label>
              <input {...register('coLawyerInCase')} type="text"
                className="w-full px-4 py-2 border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-900"
                placeholder="نام وکیل (اختیاری)" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-900 mb-2">مبلغ کل پرونده (تومان) *</label>
            ...
            <div className="border border-zinc-200 rounded-lg p-4 space-y-3">
                ...
                <div key={field.id} className="flex items-start gap-3 bg-zinc-50 p-3 rounded-lg">
                  <div className="flex-1">
                    ...
                    <input type="number"
                      ...
                      className="w-full px-3 py-2 border border-zinc-300 rounded-lg" placeholder="مبلغ قسط" />
                  </div>
                  <div className="flex-1">
                    <label className="text-xs text-zinc-600 block mb-1">سررسید (اختیاری)</label>
              ...
              {remainingAmount !== 0 && (
                <p className="text-sm text-red-600">
                  ⚠️ مجموع اقساط ({totalInstallments.toLocaleString()}) باید برابر مبلغ کل ({watchTotalAmount?.toLocaleString()}) باشد
                </p>
              )}
            </div>
          )}
        ...
        {/* توضیحات */}
        <div>
          <label className="block text-sm font-medium text-zinc-900 mb-2">توضیحات</label>
